...
describe('Phase 2.3: Search + memory.query Tools Redesign', () => {
    ...
    describe('search — space REQUIRED', () => {
        ...
        test('2.3.2 search with space: "*" searches ALL spaces', async () => {
            ...
            await store.addMemory('projects/mind', 'auth-mem', 'JWT auth implementation', { tags: ['test'] });
            // Add memory to projects/other
            await store.addMemory('projects/other', 'auth-other', 'OAuth2 implementation', { tags: ['test'] });
        ...
        test('2.3.4 search returns search_method in response', async () => {
            await store.addMemory('projects/mind', 'test-mem', 'Test content about auth', { tags: ['test'] });
        ...
        test('2.3.5 search with simple query (no operators) works', async () => {
            await store.addMemory('projects/mind', 'my-memory', 'Content about typescript', { tags: ['test'] });
        ...
        test('2.3.6 search with "exact phrase" (quoted) matches exact phrase', async () => {
            await store.addMemory(
                'projects/mind',
                'exact-phrase',
                'This is an exact phrase match test',
                { tags: ['test'] }
            );
            await store.addMemory(
                'projects/mind',
        ...
        test('2.3.7 search with prefix* works', async () => {
            await store.addMemory('projects/mind', 'typescript-guide', 'Guide to typescript', { tags: ['test'] });
            await store.addMemory('projects/mind', 'typescript-ref', 'Reference for typescript', { tags: ['test'] });
