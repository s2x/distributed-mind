...
const MemoryPromoteSchema = z.object({
  id: z.number().describe('**Required.** Memory ID to promote. Get ID from memory_list, memory_query, or search. Promotion moves memory one tier up: T4→T3, T3→T2, T2→T1. Cannot promote if already at T1.'),
});

const MemoryDemoteSchema = z.object({
  id: z.number().describe('**Required.** Memory ID to demote. Get ID from memory_list, memory_query, or search. Demotion moves memory one tier down: T1→T2, T2→T3, T3→T4. Cannot demote if already at T4 (frozen).'),
});

const MemoryPinSchema = z.object({
  id: z.number().describe('**Required.** Memory ID to pin. Get ID from memory_list, memory_query, or search. Pinned memories: 1) Do NOT auto-promote on read, 2) Are immune to LRU eviction. Use for critical memories.'),
});

const MemoryUnpinSchema = z.object({
  id: z.number().describe('**Required.** Memory ID to unpin. Get ID from memory_list, memory_query, or search. Unpinned memories resume normal tier behavior: auto-promote on read, eligible for LRU eviction.'),
});
