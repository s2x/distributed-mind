...
const CheckpointSetSchema = z.object({
  space: z.string().describe('**Required.** Working space name (NOT the checkpoint space — the space you are working in). Example: "projects/mind" or "my-project". This creates a hidden space: "my-project:sessions".'),
  goal: z.string().describe('**Required.** Current goal or task. What are you actively working on? This displays to recovered sessions.'),
  pending: z.string().describe('**Required.** What remains to be done. Remaining tasks, unfinished work. This helps recover session context.'),
  notes: z.string().optional().describe('Optional. Additional context, links, or notes. Not displayed in recovery by default.'),
  relatedMemoryIds: z.array(z.number()).optional().describe('Optional. Memory IDs to link to this checkpoint. Useful for tracking related decisions, bugs, or context memories. Get IDs from memory_list/query/search.'),
});

const CheckpointCompleteSchema = z.object({
  space: z.string().describe('**Required.** Working space name (same as used in checkpoint_set). This identifies which checkpoint space to use.'),
  checkpointId: z.number().describe('**Required.** ID of the checkpoint to mark complete. Get ID from checkpoint_list or checkpoint_recover. This moves the checkpoint from active to completed.'),
  whatWasDone: z.string().describe('**Required.** Summary of what was accomplished. This is recorded in the checkpoint history and helps track progress over time.'),
});

const CheckpointRecoverSchema = z.object({
  space: z.string().describe('**Required.** Working space name to recover checkpoint from. Looks for hidden space: "space-name:sessions".'),
  includeHistory: z.boolean().optional().describe('Optional. Include completed (historical) checkpoints in results. Default: false — only returns active checkpoint. Set true to see full history.'),
});

const CheckpointListSchema = z.object({
  space: z.string().describe('**Required.** Working space name to list checkpoints from. Looks in hidden space: "space-name:sessions".'),
  status: z.enum(['active', 'completed', 'all']).optional().describe('Optional. Filter by checkpoint status: "active" (default, in-progress), "completed" (done), "all" (both). Default: "active".'),
});
