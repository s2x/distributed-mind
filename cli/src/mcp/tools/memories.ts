...
const MemoryAddSchema = z.object({
  space: z.string().describe('**Required.** Space to add memory to. Must exist — create with space_create first. Examples: projects/mind, user/preferences, sessions/mind.'),
  name: z.string().describe('**Required.** Memory name/title. Use descriptive, searchable names. Example: "JWT auth implementation", "Fixed N+1 query".'),
  content: z.string().describe('**Required.** Memory content. See system_instructions for recommended format (**What**, **Why**, **Where**, **Learned**).'),
  tags: z.array(z.string()).optional().describe('Optional tags. Use: cat:decision, cat:bugfix, cat:pattern, cat:discovery, cat:preference, cat:config. List existing tags first to avoid duplicates.'),
  tier: z.number().int().min(1).max(3).optional().describe('Optional. Tier: 1=hot (max 25), 2=warm (default, max 50), 3=cold (max 100). Omit for default (T2).'),
});
...
const MemoryGetByIdSchema = z.object({
  id: z.number().describe('**Required.** Memory ID (numeric). Get IDs from memory_list, memory_query, or search results.'),
});

const MemoryListSchema = z.object({
  space: z.string().describe('**Required.** Space to list memories from. Must exist.'),
  tier: z.number().int().min(1).max(4).optional().describe('Optional. Filter by tier: 1 (hot), 2 (warm), 3 (cold), 4 (frozen). Default: lists T1+T2.'),
...
const MemoryUpdateSchema = z.object({
  ...
  content: z.string().optional().describe('Optional. New content. If omitted, keeps current content. Note: Updating content updates the changed_at timestamp.'),
});

const MemoryDeleteSchema = z.object({
  space: z.string().describe('**Required.** Space containing the memory. Must exist.'),
  name: z.string().describe('**Required.** Memory name to delete. Case-sensitive. This is PERMANENT — there is no undo.'),
});

const MemoryReadSchema = z.object({
  space: z.string().describe('**Required.** Space containing the memory. Must exist.'),
  name: z.string().describe('**Required.** Memory name to read. Case-sensitive. Reading: 1) Returns content, 2) Records access, 3) Auto-promotes one tier up unless pinned or at T1.'),
});
...
const MemoryTagAddSchema = z.object({
  ...
  tag: z.string().describe('**Required.** Tag to add. List existing tags first with memory_tags_list to avoid duplicates. Recommended: cat:decision, cat:bugfix, cat:pattern, cat:discovery, cat:preference, cat:config.'),
});
...
const MemoryQuerySchema = z.object({
  space: z.string().optional().describe('Optional. Filter by space name. Example: "projects/mind" or "user/preferences".'),
  tag: z.string().optional().describe('Optional. Filter by tag (do not include #). Example: "cat:decision".'),
  tier: z.number().int().min(1).max(4).optional().describe('Optional. Filter by tier: 1 (hot), 2 (warm), 3 (cold), 4 (frozen).'),
  from: z.string().optional().describe('Optional. Changed date lower bound. Format: "YYYY-MM-DD" or "YYYY-MM-DD HH:MM:SS" or ISO datetime.'),
  to: z.string().optional().describe('Optional. Changed date upper bound. Format: same as from.'),
  limit: z.number().int().min(1).max(500).optional().describe('Optional. Number of results per page. Default: 25. Max: 500.'),
  offset: z.number().int().min(0).optional().describe('Optional. Zero-based offset for pagination. Default: 0. Use offset + limit for next page.'),
});
