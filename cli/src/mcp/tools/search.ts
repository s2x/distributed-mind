...
const SearchSchema = z.object({
  query: z
    ...
    .min(1)
    .describe(
      'Search query using FTS5 syntax. Supports: quoted phrases ("exact match"), AND/OR/NOT operators, prefix matching (bug*).'
    ),
...
const SEARCH_TOOL_DESCRIPTIONS: Record<string, string> = {
  search:
    'Full-text search across all memories. Use FTS5 syntax (e.g., "auth AND jwt", "bug*"). Space is required; use "*" for all spaces.',
  memory_query:
    'Query memories by metadata/date with pagination. Use space="*" for all spaces. Supports filtering by tag, tier, and date range.',
  status:
  ...
  return {
    search: {
      ...
      handler: async (args: unknown) => {
        ...
        return {
          content: [
            { type: 'text', text: `Found ${results.length} result(s) for "${parsed.query}".` },
          ],

          results: results.map(({ id: _id, ...rest }: any) => rest),
          search_method,
        };
      },
    },
      ...
      handler: async (args: unknown) => {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Found ${memories.length} memory/memories (total: ${totalFiltered.length}).`,
            },
          ],

          memories: memories.map(({ id: _id, ...rest }) => rest),
