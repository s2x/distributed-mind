...
const LinkCreateSchema = z.object({
  sourceId: z.number().describe('**Required.** Source memory ID (the "from" memory). Get ID from memory_list, memory_query, or search. Cannot link a memory to itself.'),
  targetId: z.number().describe('**Required.** Target memory ID (the "to" memory). Get ID from memory_list, memory_query, or search. Cannot link to self.'),
  label: z.string().optional().describe('Optional. Relationship label. Common: "related" (default), "depends-on", "fixes", "blocks", "duplicates". Custom labels allowed.'),
});
...
const LinksListSchema = z.object({
  memoryId: z.number().describe('**Required.** Memory ID to list links for. Returns both outgoing links (where this memory is source) and incoming links (where this memory is target).'),
});
