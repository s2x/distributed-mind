...
const SpaceCreateSchema = z.object({
  name: z.string().describe('**Required.** Space name. Use hierarchical format: projects/project-name, user/preferences, global/config, sessions/project-name. See system_instructions for conventions.'),
  description: z.string().describe('Description of the space purpose. Example: "Project-specific decisions and patterns".'),
  tags: z.array(z.string()).optional().describe('Optional tags. Recommended: type:project, type:user, type:config, type:learning, type:session. List existing tags first to avoid duplicates.'),
});
...
const SpaceGetSchema = z.object({
  name: z.string().describe('**Required.** Space name to retrieve. Must exist — create with space_create first if needed.'),
});
...
const SpaceRenameSchema = z.object({
  oldName: z.string().describe('**Required.** Current space name. Must exist. Note: All memories in this space will move to the new name.'),
  newName: z.string().describe('**Required.** New space name. Must not already exist. Use same hierarchical format as oldName.'),
});

const SpaceDeleteSchema = z.object({
  name: z.string().describe('**Required.** Space name to delete. This is PERMANENT — all memories in the space will be deleted. There is no undo.'),
});

const SpaceTagAddSchema = z.object({
  space: z.string().describe('**Required.** Space to tag. Must exist.'),
  tag: z.string().describe('**Required.** Tag to add. List existing tags first with memory_tags_list to avoid duplicates. Recommended: type:project, type:user, cat:decision, cat:bugfix, cat:pattern, cat:discovery, cat:preference, cat:config.'),
});
