...
type SupportedAgent = 'claude-code' | 'opencode' | 'codex' | 'cursor' | 'windsurf' | 'gemini-cli';
type RoadmapAgent = 'vscode' | 'antigravity' | 'kiro';
type ExperimentalAgent = 'openclaw';
type Agent = SupportedAgent | RoadmapAgent | ExperimentalAgent;

type CapabilityLevel = 'L1_MCP' | 'L2_INSTRUCTIONS' | 'L3_HOOKS';
...
type CapabilityMap = Record<CapabilityLevel, CapabilityDeclaration>;
...
function getAgentConfig(agent: SupportedAgent): AgentConfig {
    ...
    const map: Record<SupportedAgent, AgentConfig> = {
        'claude-code': {
            name: 'Claude Code',
...
function formatCapabilityLine(level: CapabilityLevel, capability: CapabilityDeclaration): string {
...
export function getAgentCapabilities(agent: Agent): CapabilityMap {
    ...
    return getAgentConfig(agent as SupportedAgent).capabilities;
}

export function getAgentCapabilityMatrix(): CapabilityMatrixEntry[] {
    const agents: SupportedAgent[] = ['claude-code', 'opencode', 'codex', 'cursor', 'windsurf', 'gemini-cli'];
    const configuredAgents = agents.map((agent) => {
