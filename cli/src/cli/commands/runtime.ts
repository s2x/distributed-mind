...
import { runSetup, listAgents, startMcpDetached, stopMcp, startServeDetached, stopServe, statusServers } from '../setup';
...
const SERVE_START = new ArgParser(
  ['serve|server|web', 'start'],
  'Starts web HTTP server',
  [
    { name: 'detached', hasValue: false, description: 'run in background' },
...
const UPDATE = new ArgParser(
  ['update'],
  'Updates mind from GitHub releases',
  [
...
async function runMcpForeground(flags: Record<string, string | boolean>, store: MindStore): Promise<void> {
...
export const runtimeGroup: CommandGroup = {
  name: 'Server',
  helpEntries: [SERVER_STATUS, MCP_STDIO, MCP_START, MCP_STOP, SERVE_START, SERVE_STOP, SETUP_AGENT, UPDATE],
  commands: [
    {
