    ...
    commands: [
        {
            ...
            execute: async (args, store, logger) => {
                ...
                logger.logInfo(style(`🔍 ${results.length} result(s) for "${query}":`, ['bold', 'magenta']));
        ...
        },
        {
            ...
            execute: async (args, store, logger) => {
                ...
                const nextOffset = results.length === effectiveLimit ? String(effectiveOffset + effectiveLimit) : 'N/A';
                logger.logInfo(
                    style(
