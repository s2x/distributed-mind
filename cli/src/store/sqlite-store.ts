...
export function createSqliteStore(dbPath: string): MindStore {
    ...
    function promote(id: number): void {
        ...
        const ts = now();
        db.run('UPDATE memories SET tier = ?, updated_at = ?, changed_at = ? WHERE id = ?', [toTier, ts, ts, id]);
    }
    ...
    function pin(id: number): void {
        requireMemory(id);
        const ts = now();
        db.run('UPDATE memories SET pinned = 1, updated_at = ?, changed_at = ? WHERE id = ?', [ts, ts, id]);
    }

    function unpin(id: number): void {
        requireMemory(id);
        const ts = now();
        db.run('UPDATE memories SET pinned = 0, updated_at = ?, changed_at = ? WHERE id = ?', [ts, ts, id]);
    }
    ...
    function queryLogs(filter?: {
        ...
        if (filter?.search) {
            ...
            conditions.push('(operation LIKE ? OR input_data LIKE ? OR output_data LIKE ? OR error_message LIKE ?)');
        ...
        // Get total count
        const countRow = db.query(`SELECT COUNT(*) as total FROM logs ${whereClause}`).get(...params) as {
            total: number;
        };
...
export function subscribeToLogs(sessionId: string, controller: SseController, filter?: string): void {
