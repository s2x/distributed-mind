...
describe('Phase 1.4 — Tags validation: createSpace', () => {
  test('createSpace without tags should throw error', async () => {
    ...
    expect(() => (store as any).createSpace('test', 'Test space')).toThrow(
      /tags? (is|are) required|empty|not provided/i
    );
  });
...
describe('Phase 1.4 — Tags validation: addMemory', () => {
  test('addMemory without tags option should throw error', async () => {
    ...
    await expect((store as any).addMemory('test', 'mem1', 'content')).rejects.toThrow(
      /tags? (is|are) required|empty|not provided/i
    );
  });
  ...
  test('addMemory with empty tags array should throw error', async () => {
    ...
    await expect((store as any).addMemory('test', 'mem1', 'content', { tags: [] })).rejects.toThrow(
      /tags? (is|are) required|empty|not provided/i
    );
  });
});
