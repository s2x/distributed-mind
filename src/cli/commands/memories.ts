...
const READ = new ArgParser(
  ['read|r', p('space'), p('name')],
  'Reads a memory (bumps access + auto-promotes)'
);
const EDIT = new ArgParser(
  ['edit|e', p('space'), p('name'), p('content')],
  'Edits a memory content'
);
...
const UNTAG_MEMORY = new ArgParser(
  ['untag', p('space'), p('name'), p('tag')],
  'Removes a tag from a memory'
);
  ...
  commands: [
    ...
    },
    {
      ...
      execute: async (args, store, logger) => {
        ...
        logger.logInfo(
          style(`🛸 ${space} › ${memory.name}`, ['bold', 'blue']) +
            ` [${tierLabel(memory.tier)}]${pin}`
        );
