...
const PIN = new ArgParser(
  ['pin', p('space'), p('name')],
  'Pins a memory (immune to auto-promotion)'
);
  ...
  commands: [
    ...
    },
    {
      ...
      execute: async (args, store, logger) => {
        ...
        const memory = await store.getMemory(space, name);
        if (!memory) throw new Error(`Memory "${name}" not found in space "${space}"`);
        await store.pin(memory.id);
        logger.logInfo(
          style(`📌 "${name}" pinned at ${tierLabel(memory.tier)}`, ['bold', 'green'])
        );
      },
    },
    {
