...
- **Layout:** **`cli/`** contains CLI, MCP, and API server code plus backend/CLI tests. **`web/`** contains frontend source (`web/src`), styles (`web/styles`), static assets (`web/assets`), static HTML shell (`web/public`), and web-specific tests (`web/test`). **`scripts/`** contains E2E test scripts.
...
- **Web/API:** The single canonical HTTP server is `cli/src/api/server.ts` (via `mind serve`), and it serves API routes plus static web files from the `web/` tree.
...
| Setup/runtime helpers | `cli/src/cli/setup.ts`                | Agent setup + detached process management helpers for MCP/web servers. Uses a capability-driven adapter model (L1 MCP, L2 instruction injection, L3 hooks automation) with explicit status (`supported`/`unsupported`/`unverified`) and visible fallback diagnostics. OpenCode/Claude/Codex L2 protocol injection now renders from a single canonical template source at setup-time (agent-specific wording via internal renderer), while preserving idempotent/non-destructive managed-file behavior. Cursor setup preserves existing L1/L3 behavior and provisions global L3 hooks automation via managed `~/.cursor/hooks.json` entries and managed executable script artifacts in `~/.cursor/hooks/`. |
| Shared capabilities   | `cli/src/cli/capabilities.ts`         | Canonical capability declarations (L1/L2/L3 status/confidence/evidence/fallback) reused by setup diagnostics and checkpoint recovery orchestration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Recovery pack         | `cli/src/checkpoint/recovery-pack.ts` | Builds and renders integrated recovery payload for `checkpoint recover` (checkpoint state, recent context hits, capability profile, degradation/guidance) in `text`, `md`, and `json` formats.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| MCP server            | `cli/src/mcp/server.ts`               | MCP stdio server using `@modelcontextprotocol/sdk`. Exposes tools across spaces, memories, links, search, checkpoints, and system categories.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| MCP tools             | `cli/src/mcp/tools/`                  | Tool implementations: `spaces.ts`, `memories.ts`, `links.ts`, `search.ts`, `checkpoint.ts`, `system.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| API server            | `cli/src/api/server.ts`               | Bun HTTP server that serves `/api/*` routes and static web files from `web/` (SPA shell in `web/public/index.html`, frontend modules in `web/src`, styles in `web/styles`, assets in `web/assets`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| API router            | `cli/src/api/router.ts`               | Route matcher/dispatcher for API endpoints.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| API routes            | `cli/src/api/routes/*.ts`             | Atomic REST route declarations grouped by domain (`spaces`, `memories`, `search`, `status`, `logs`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- **Config / storage path:** `cli/src/config.ts` resolves `CONFIG.dbPath` from `MIND_DB_PATH` env var (full path override) or `MIND_DATA_DIR` env var + `mind.db` (defaults to `data/` at repo root). The web server uses `MIND_DATA_DIR` (or `/data` in Docker). `data/` is in `.gitignore`. HTTP idle timeouts are configurable via `MIND_MCP_IDLE_TIMEOUT` (default 120s) and `MIND_API_IDLE_TIMEOUT` (default 30s). Log retention is configurable via `MIND_LOG_RETENTION_MINUTES` (default 10080 minutes = 7 days).
...
- **Protocol sources:** Setup L2 managed protocol payloads for OpenCode/Claude/Codex are rendered from the canonical template `cli/src/resources/protocols/mind-memory-protocol.template.md` using `cli/src/helpers/template-renderer.ts` and `cli/src/cli/memory-protocol.ts`. MCP `system_instructions` is rendered through the same canonical template pipeline by `cli/src/cli/system-instructions.ts` from `cli/src/resources/protocols/mind-system-instructions.md` (empty-context render for deterministic text output). The template renderer operates in strict mode: unresolved placeholders/conditionals (or leftover template tokens) throw errors for all render paths.
- **Testing:** Backend/CLI tests live in `cli/test/`. Web-specific tests live in `web/test/`. Both use `bun:test` and rely on:
    ...
    - Backend/CLI test files include: `cli/test/mind-store.spec.ts` (store-level, including graph retrieval), `cli/test/api-routes.spec.ts` (HTTP route-level behavior), `cli/test/command-executor.spec.ts` (CLI-level), `cli/test/mcp-tools.spec.ts` (MCP tools), `cli/test/setup-opencode.spec.ts`, `cli/test/setup-capabilities.spec.ts`, `cli/test/memory-protocol-renderer.spec.ts`, `cli/test/system-tools.spec.ts`, `cli/test/template-renderer.spec.ts`, and `cli/test/arg-parser.spec.ts`.
...
| Checkpoint set      | `checkpoint set`      | `cp set`                                    | `<space>` `<goal>` `<pending>` | `--notes`                                                             | Create or update an active checkpoint in the project space.                                                                                                                     |
| Checkpoint complete | `checkpoint complete` | `cp complete`, `checkpoint done`, `cp done` | `<space>` `<name>` `<what>`    | —                                                                     | Complete a checkpoint, mark tags, and demote tier.                                                                                                                              |
| Checkpoint recover  | `checkpoint recover`  | `cp recover`                                | `<space>`                      | `--history`, `--format`, `--agent`                                    | Recover integrated Recovery Pack via checkpoint interface (optionally include history; output `text\|md\|json`; capability profile selected by agent).                          |
...
The MCP server exposes 20 tools for agent integration:

#### Spaces (5 tools)

| Tool           | Description                                          |
| -------------- | ---------------------------------------------------- |
| `space.create` | Create a new space (requires `tags`)                 |
| `space.list`   | List all spaces                                      |
| `space.get`    | Get a space by name; returns `hot_memories` preview (T1 + T2) |
| `space.update` | Update a space description                           |
| `space.delete` | Delete a space and all its memories                  |
...
| Tool           | Description                          |
| -------------- | ------------------------------------ |
| `link.create`  | Create a directional link between two memories |
| `link.delete`  | Delete a directional link             |

#### Checkpoint (4 tools)

| Tool              | Description                                        |
| ----------------- | -------------------------------------------------- |
| `checkpoint.save` | Create or update a checkpoint (renamed from `set`) |
| `checkpoint.load` | Recover latest active checkpoint (renamed from `recover`) |
| `checkpoint.list` | List checkpoints for a space                      |
| `checkpoint.done` | Mark a checkpoint complete (renamed from `complete`) |

#### System (1 tool)

| Tool            | Description                      |
| --------------- | -------------------------------- |
| `system.status` | Get storage status               |
